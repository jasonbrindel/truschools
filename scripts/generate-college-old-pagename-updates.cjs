...
// Load redirects
const redirects = JSON.parse(fs.readFileSync('./data/college-redirects.json', 'utf8'));
console.log(`Loaded ${redirects.length.toLocaleString()} redirects`);

// Group by page_name to avoid duplicates
...
for (const [pageName, oldSlug] of pageNameToOldSlug) {
  ...
  sqlLines.push(`UPDATE colleges SET old_page_name = '${escapedOldSlug}' WHERE page_name = '${escapedPageName}';`);
}

// Write to file
const sqlContent = sqlLines.join('\n');
fs.writeFileSync('./data/update-college-old-page-names.sql', sqlContent);
console.log(`Written ${sqlLines.length.toLocaleString()} UPDATE statements to data/update-college-old-page-names.sql`);
...
fs.mkdirSync('./data/college-old-pagename-batches', { recursive: true });
for (let i = 0; i < batches.length; i++) {
  const batchNum = String(i + 1).padStart(3, '0');
  fs.writeFileSync(`./data/college-old-pagename-batches/batch-${batchNum}.sql`, batches[i]);
}
console.log(`Split into ${batches.length} batch files in data/college-old-pagename-batches/`);
